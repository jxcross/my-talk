...
def script_creation_page():
    ...
    if st.button("🚀 스크립트 생성하기", type="primary", key="generate_script"):
        ...
        if results:
            ...
            with col1:
                if st.button("💾 로컬 저장", type="secondary"):
                    save_to_local_db(results, input_content, input_method, category)
            
            with col2:
                if st.session_state.google_drive_enabled and st.button("☁️ Google Drive 저장", type="primary"):
                    save_to_google_drive(results, gdrive_manager)
        else:
...
def display_results(results, selected_versions):
    ...
    tab_names = [version_names[v] for v in selected_versions]
        ...
        with tabs[i]:
            ...
            if script_key in results:
                ...
                if audio_key in results and results[audio_key]:
                    st.audio(results[audio_key], format='audio/mp3')
                else:
                    # 브라우저 TTS 폴백
...
def save_to_local_db(results, input_content, input_method, category):
    """로컬 데이터베이스에 저장"""
    try:
        db = EnhancedDatabase()
        ...
        script_id = db.create_script_project(
            title=results['title'],
            original_content=results['original_script'],
        ...
        st.success(f"✅ 로컬 저장 완료! (ID: {script_id})")
        
    except Exception as e:
        st.error(f"로컬 저장 실패: {str(e)}")

def save_to_google_drive(results, gdrive_manager):
    """Google Drive에 저장"""
    ...
    try:
        with st.spinner("Google Drive에 저장 중..."):
            # 저장할 파일 목록 준비
            files_to_save = []
            
            # 원본 스크립트와 번역
            original_script_path = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8')
            original_script_path.write(results['original_script'])
            original_script_path.close()
            files_to_save.append({
                'type': 'original_script',
                'path': original_script_path.name,
                'name': 'original_script.txt'
            })
            
            if 'korean_translation' in results:
                translation_path = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8')
                translation_path.write(results['korean_translation'])
                translation_path.close()
                files_to_save.append({
                    'type': 'korean_translation',
                    'path': translation_path.name,
                    'name': 'korean_translation.txt'
                })
            
            # 각 버전별 파일들
            versions = ['original', 'ted', 'podcast', 'daily']
            for version in versions:
                script_key = f"{version}_script" if version != 'original' else 'original_script'
                audio_key = f"{version}_audio" if version != 'original' else 'original_audio'
                
                # 음성 파일
                if audio_key in results and results[audio_key] and os.path.exists(results[audio_key]):
                    files_to_save.append({
                        'type': f'{version}_audio',
                        'path': results[audio_key],
                        'name': f'{version}_audio.mp3'
                    })
                
                # 버전별 스크립트 (원본 제외, 이미 저장됨)
                if version != 'original' and script_key in results:
                    version_script_path = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8')
                    version_script_path.write(results[script_key])
                    version_script_path.close()
                    files_to_save.append({
                        'type': f'{version}_script',
                        'path': version_script_path.name,
                        'name': f'{version}_script.txt'
                    })
            
            # Google Drive에 업로드
            success = gdrive_manager.save_project_files(
                script_id=0,  # 임시 ID (실제 DB 저장 시 업데이트 필요)
            ...
            for file_data in files_to_save:
                if file_data['type'].endswith('_script') or file_data['type'] == 'korean_translation':
                    try:

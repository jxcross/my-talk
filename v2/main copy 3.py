...
class EnhancedDatabase:
    ...
    def create_script_project(self, title, original_content, korean_translation='', category='general', input_type='text', input_data=''):
        """스크립트 프로젝트 생성 (단순화)"""
        try:
            # 데이터 검증
            if not title.strip():
                title = f"Script_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            c.execute('''
                INSERT INTO scripts (title, original_content, korean_translation, category, input_type, input_data)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (title.strip(), original_content.strip(), korean_translation, category, input_type, input_data))
            
            script_id = c.lastrowid
            conn.commit()
            
            # 즉시 확인
            c.execute('SELECT id, title FROM scripts WHERE id = ?', (script_id,))
            result = c.fetchone()
            conn.close()
            ...
            if result:
                ...
                raise Exception(f"저장 후 확인 실패: ID {script_id}")
                
        except Exception as e:
            raise Exception(f"스크립트 저장 오류: {str(e)}")
    ...
    def add_practice_version(self, script_id, version_type, content, audio_path=''):
        ...
        try:
            ...
            raise Exception(f"{version_type} 버전 저장 오류: {str(e)}")
...
def settings_page():
    ...
    with st.expander("🔧 시스템 상태"):
        ...
        if os.path.exists(db_path):
            ...
            st.success(f"✅ 데이터베이스 파일 존재: {file_size} bytes")
            
            # 직접 쿼리 테스트
            if st.button("📋 저장된 모든 스크립트 보기"):
                try:
                    conn = sqlite3.connect(db_path)
                    c = conn.cursor()
                    c.execute("SELECT id, title, category, created_at FROM scripts ORDER BY created_at DESC")
                    all_scripts = c.fetchall()
                    
                    if all_scripts:
                        st.write(f"총 {len(all_scripts)}개의 스크립트:")
                        for script in all_scripts:
                            st.write(f"• ID {script[0]}: **{script[1]}** ({script[2]}) - {script[3][:16]}")
                    else:
                        st.write("저장된 스크립트가 없습니다.")
                    
                    conn.close()
                except Exception as e:
                    st.error(f"쿼리 오류: {str(e)}")
        else:
...
def practice_page():
    ...
    # 갱신 트리거 확인
    if hasattr(st.session_state, 'refresh_tabs'):
        st.success(f"🆕 새로운 스크립트가 저장되었습니다! ({st.session_state.refresh_tabs[:19]})")
    ...
    try:
        ...
        if not scripts:
            ...
            # 실시간 데이터베이스 상태
            with st.expander("📊 데이터베이스 상태"):
                conn = sqlite3.connect(db.db_path)
                c = conn.cursor()
                
                c.execute("SELECT COUNT(*) FROM scripts")
                count = c.fetchone()[0]
                st.write(f"스크립트 개수: {count}")
                
                if count > 0:
                    c.execute("SELECT id, title, created_at FROM scripts ORDER BY created_at DESC LIMIT 3")
                    recent = c.fetchall()
                    st.write("최근 스크립트:")
                    for r in recent:

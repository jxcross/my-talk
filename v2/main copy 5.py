...
def script_creation_page():
                ...
                with col3:
                    if st.button("복구", key=f"restore_{backup['id']}"):
                        ...
                        if restored_data:
                            ...
                            st.session_state.restored_input_method = restored_data.get('input_method', '텍스트')
                            st.session_state.restored_category = restored_data.get('category', '일반')
                            st.success("✅ 백업 복구 완료!")
                            st.rerun()
        else:
    ...
    if hasattr(st.session_state, 'current_project') and st.session_state.current_project:
        ...
        backup_id = getattr(st.session_state, 'current_backup_id', None)
        input_content = getattr(st.session_state, 'restored_input_content', '')
        input_method = getattr(st.session_state, 'restored_input_method', '텍스트')
        category = getattr(st.session_state, 'restored_category', '일반')
        ...
        with col1:
            if st.button("💾 로컬 저장", type="primary", key="save_local_main"):
                success = save_to_files_and_db(results, input_content, input_method, category)
                if success:
                    # 백업 정리
                    if backup_id:
                        cleanup_temp_backup_fixed(backup_id)
                        for attr in ['current_backup_id', 'current_project', 'restored_input_content', 'restored_input_method', 'restored_category']:
    ...
    if st.button("🚀 스크립트 생성하기", type="primary"):
        ...
        with progress_container.container():
            ...
            if original_response:
                ...
                with col1:
                    if st.button("💾 로컬 저장", type="primary", key="save_local_main2"):
                        success = save_to_files_and_db(results, input_content, input_method, category)
                        if success:
                            # 백업 정리
                            if backup_id:
                                cleanup_temp_backup_fixed(backup_id)
                                for attr in ['current_backup_id', 'current_project']:
                                    if hasattr(st.session_state, attr):
                                        delattr(st.session_state, attr)
                            st.balloons()
                            st.info("💡 '연습하기' 또는 '내 스크립트' 탭에서 저장된 내용을 확인하세요!")

...
def extract_role_dialogues(text, version_type):
    ...
    try:
        ...
        if version_type == 'podcast':
            ...
            if host_texts:
                st.write(f"🔍 Host 첫 대사 미리보기: {host_texts[0][:50]}...")
            if guest_texts:
                st.write(f"🔍 Guest 첫 대사 미리보기: {guest_texts[0][:50]}...")
            ...
            if a_texts:
                st.write(f"🔍 Person A 첫 대사 미리보기: {a_texts[0][:50]}...")
            if b_texts:
                st.write(f"🔍 Person B 첫 대사 미리보기: {b_texts[0][:50]}...")
    ...
    try:
        ...
        if version_type in ['podcast', 'daily']:
            ...
            if role1_text:
                ...
                st.write(f"📝 {role1_key.upper()} 텍스트: {role1_text[:100]}...")
            ...
            if role2_text:
                ...
                st.write(f"📝 {role2_key.upper()} 텍스트: {role2_text[:100]}...")
            ...
            if len(audio_files) >= 1:  # 최소 1개 이상만 있어도 시도

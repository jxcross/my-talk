"""
MyTalk - Simplified Version with OpenAI Multi-Voice TTS
주요 기능:
1. 로컬 파일 시스템 저장
2. OpenAI TTS API 통합 (Multi-Voice: 음성언어-1, 음성언어-2)
3. 2인 대화 형식 지원 (Host/Guest, A/B 역할별 음성 배정)
4. 이미지 설명 기반 스크립트 생성
5. 향상된 음성 변환 (메타 정보 제거)
"""
...
def extract_role_dialogues(text, version_type):
    ...
    try:
        ...
        if version_type == 'podcast':
            ...
            host_lines = [item[1] for item in dialogue_sequence if item[0] == 'host']
            guest_lines = [item[1] for item in dialogue_sequence if item[0] == 'guest']
            
            return {
                'host': ' '.join(host_lines),
                'guest': ' '.join(guest_lines),
            ...
            # 역할별로 분리된 텍스트와 순서 정보 반환
            a_lines = [item[1] for item in dialogue_sequence if item[0] == 'a']
            b_lines = [item[1] for item in dialogue_sequence if item[0] == 'b']
...
def generate_multi_voice_audio(text, api_key, voice1, voice2, version_type):
    ...
    try:
        ...
        if version_type in ['podcast', 'daily']:
            ...
            if role_dialogues and 'sequence' in role_dialogues:
                audio_files = {}
                
                # 첫 번째 역할 (Host/A)
                role1_key = 'host' if version_type == 'podcast' else 'a'
                if role_dialogues[role1_key]:
                    st.write(f"🎙️ {role1_key.upper()} 음성 생성 중...")
                    audio1 = generate_audio_with_openai_tts(
                        role_dialogues[role1_key], api_key, voice1
                    )
                    if audio1:
                ...
                if role_dialogues[role2_key]:
                    st.write(f"🎤 {role2_key.upper()} 음성 생성 중...")
                    audio2 = generate_audio_with_openai_tts(
                        role_dialogues[role2_key], api_key, voice2
                    )
                    if audio2:
